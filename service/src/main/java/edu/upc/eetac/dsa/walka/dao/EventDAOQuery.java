...
public interface EventDAOQuery {
    ...
    public final static String GET_EVENTS_BY_USERID = "SELECT hex(e.id) as id, e.title, hex(e.creator) as creator, e.location, e.notes, e.startdate, e.enddate, e.creation_timestamp, e.last_modified FROM events AS e INNER JOIN user_events AS ue ON e.id = ue.idevent WHERE ue.iduser = unhex(?)";
   ...
   /**Corregir*/public final static String GET_EVENTS_BETWEEN = "SELECT e.* FROM events AS e INNER JOIN user_events AS ue ON e.id = ue.idevent WHERE ue.iduser = unhex(?) AND (e.startdate BETWEEN ? AND ?)";
    public final static String GET_EVENTS_BY_MONTH = "SELECT e.* FROM events AS e INNER JOIN user_events AS ue ON e.id = ue.idevent WHERE ue.iduser = unhex(?) AND (e.startdate BETWEEN ? AND date_add(?, INTERVAL 1 MONTH))";
    public final static String GET_EVENTS_BY_DAY = "SELECT e.* FROM events AS e INNER JOIN user_events AS ue ON e.id = ue.idevent WHERE ue.iduser = unhex(?) AND (e.startdate BETWEEN ? AND date_add(?, INTERVAL 1 DAY))";
